...
fn dimension_in_group_by(dim: &str, group_by_cols: &[String]) -> bool {
        ...
        for modifier in modifiers {
            if let ContextModifier::Set(dim, expr) = modifier {
                ...
                if expr_mentions_identifier(expr, dim_name)
                    && !dimension_in_group_by(dim_name, group_by_cols)
                {
                    return Some(format!(
...
pub fn expand_aggregate_with_at(sql: &str) -> AggregateExpandResult {
    ...
    if let Some(error) = validate_set_expression_requirements(&at_patterns, &group_by_cols) {
        return AggregateExpandResult {
            had_aggregate: true,
            expanded_sql: sql.to_string(),
            error: Some(error),
